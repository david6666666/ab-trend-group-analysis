...
这种情况下，如果还把它们硬算成同一个 group，肉眼上会觉得“不合理”。

所以脚本会在趋势分组之后，再做一次“组内跳变检查”：

- 如果某个 group 里相邻两点的差值明显大于这条曲线平时的正常波动
- 就把这个位置再切开，单独变成新的 group
...
如果两列都用同一个跳变阈值，就很容易误判。

所以脚本的做法是：

- 对每个 sheet 的 B 列单独算一个跳变阈值
- 对每个 sheet 的 C 列再单独算一个跳变阈值
...
- 先看这列平时相邻点变化有多大
- 只有明显高于“日常波动水平”的变化，才认定为跳变

这样可以减少把正常波动误切成很多小 group。

### 7. 相似度是怎么计算的

每个 group 的相似度不是单靠一个指标决定，而是五部分组合：
...
脚本会比较两条曲线从 group 起点到终点的净变化量，并计算相对误差。

如果两条曲线都是上升，但一个涨了 100，另一个只涨了 20，那么方向虽然一致，方向相对误差仍然较大，相似度会被拉低。
...
这个指标直接比较两条曲线在 group 内每个点的数值是否接近。
...
如果两条曲线趋势和形状都接近，但整体数值差距很大，这一项会把相似度适当拉低。
...
- 方向一致性：30%
- 方向相对误差：20%，判断整体变化幅度是否接近
- 斜率接近程度：20%，判断平均变化速度是否接近
- Pearson 相关系数：15%，辅助判断形状同步性
- 数值相对误差程度：15%，判断逐点数值是否接近
...
- `direction_relative_error`：两条曲线这一段净变化量的方向相对误差
- `pearson_corr`：形状相关性
- `value_relative_error`：两条曲线这一段逐点数值的平均相对误差
